...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[]):  # pylint: disable=dangerous-default-value
    ...
    ret.openpilotLongitudinalControl = Params().get_bool("DisableRadar") and candidate in [CAR.SONATA, CAR.SONATA_HYBRID, CAR.PALISADE, CAR.SANTA_FE, CAR.IONIQ, CAR.KIA_NIRO_EV]
    ...
    if candidate in [CAR.SANTA_FE, CAR.SANTA_FE_2022]:
      ...
      ret.lateralTuning.pid.kiBP, ret.lateralTuning.pid.kpBP = [[0.], [0.]]
      ret.lateralTuning.pid.kpV, ret.lateralTuning.pid.kiV = [[0.25], [0.05]]
      if candidate not in [CAR.IONIQ_EV_2020, CAR.IONIQ]:
        ret.minSteerSpeed = 32 * CV.MPH_TO_MS
  ...
  def init(CP, logcan, sendcan):
    if CP.openpilotLongitudinalControl and Params().get_bool("DisableECU"):
      disable_ecu(logcan, sendcan, addr=0x7d0, com_cont_req=b'\x28\x83\x01') #notetoself
